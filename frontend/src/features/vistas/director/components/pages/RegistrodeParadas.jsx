...
import { Calendar, ChevronDown, MapPin, Binary, Tag, Info, X } from "lucide-react";
...
export default function RegistrodeParadas() {
  ...
  const [mesModalSeleccionado, setMesModalSeleccionado] = useState(null);
  ...
  const registrosFiltrados = registroHistorico.map((mes) => {
    const filasFiltradas = mes.registros.filter((reg) => {
      ...
      if (selectedDate) {
        ...
        const fechaFormatoISO = `${anoReg}-${mesReg}-${dia}`;
        cumpleFecha = fechaFormatoISO === selectedDate;
      }
  ...
  return (
    <div className="h-full space-y-5 text-gray-800 p-1">
        ...
        <div className="flex flex-wrap gap-3">
          ...
          <div className="relative w-48">
            ...
            <button
              ...
              className={`${estilos.filtros} text-left text-gray-400 pr-8`}
            >
                    ...
                    return (
                      <div key={mes.month}>
                        ...
                        <div className="flex items-center justify-between w-full border-b border-gray-200/70 pb-1 mb-2">
                          ...
                          <button 
                            onClick={() => setMesModalSeleccionado(mes.month)}
                            className="flex items-center gap-1 text-xs text-gray-400 hover:text-gray-600 font-medium underline underline-offset-2"
                          >
      ...
      {mesModalSeleccionado && (
        <ParadasTotalesModal 
          title="Horas totales de paradas" 
          mesSeleccionado={mesModalSeleccionado}
          onClose={() => setMesModalSeleccionado(null)}
        />
      )}
